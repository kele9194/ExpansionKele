...
Object relation
A custom UI is nothing more than a custom UserInterface with its state set to a custom UIState. In general, a UserInterface can have one active UIState (it can however hold a history of states, up to a cap, which you can go back to), that state is then currently shown in that UserInterface.
...
Notice that, in this case you're adding your UI layer below the "Mouse Text" layer, this means your UI will be drawn before that layer. You can add your custom layer next to other layers (vanilla or not) by looking for layers with other names. For a list of vanilla interface layers you can go here.
Showing the UI
...
Override the OnInitialize method. This is called when we initialize the UI when our mod loads.
Create a new UIPanel instance. This is a vanilla class that will draw a vanilla styled backdrop on this element.
Set the width and height to something bigger so we can see it.
Append the panel to our UIState. Append is a method on the UIElement class and it allows you to add child elements to that element. Children are placed relative to that element. Since UIState covers the entire screen, our UIPanel will show up in the top left origin of the screen.
Adding some text
...
Initialize the UIText.
The UIText element is now a child of the UIPanel element. The text should show in the UIPanel.
Centering element
...
Set the horizontal alignment to 50%
Set the Top position to 15 pixels. The text should be 15 pixels down relative to the UIPanel. Remember you could use VAlign for this, but it will scale as your UIPanel changes size. This may screw up things as you change the UIPanel's size.
...
This is a neat trick to set both fields to the same value.
...
Initialize a new UIPanel. Because there is no UIButton class (only UIImageButton), we will use UIPanel and UIText for this.
Set the top position slightly below our header.
Add the new OnClick event. It is important to note events will propagate through the parent-child chain. This means if we click the UIText, the click event will end up in our UIPanel (button) as well because it is the parent. For this reason we only have to add the handler to the UIPanel.
Set the text alignment so it centers in our button panel.
Append the text to our button. The button is appended to our background pannel.
...
Events
There are many events like OnLeftClick, here is a list of them:
...
By default, UI won't prevent mouse clicks from causing the player to use their currently selected item. To implement this behavior, the Player.mouseInterface bool must be set to true. This should be done in a DrawSelf method. The logic should be applied to the lowest parent that makes sense. Do not apply this to the UIState itself since UIState covers the whole screen, but rather apply it to the panels or containers added to the UIState.
...
By default, UI won't prevent the scroll wheel from shifting the selected Hotbar item. To implement this behavior, the Terraria.GameInput.PlayerInput.LockVanillaMouseScroll method should be called. This should be done in a DrawSelf method. The logic should be applied to the lowest parent that makes sense. Do not apply this to the UIState itself since UIState covers the whole screen, but rather apply it to the scrollable lists or panels added to the UIState or other containers.
...
protected override void DrawSelf(SpriteBatch spriteBatch) {
    ...
    if (list.IsMouseHovering) {
        PlayerInput.LockVanillaMouseScroll("MyMod/ScrollListB"); // The passed in string can be anything.
    }
}
Tooltip on hover
...
We define variables our custom button needs.
A public property that we can use to get and set the text backing field. More on this in point 8.
Simply return the text of our _uiText, or and empty string if it is null. (not initialized yet)
Our constructor. We must pass a text and click action. Text is of type object, modelled after the UIText class.
Create a new UIPanel as the base background. We set the size to Fill, which is equal to calling Set(0, 1f); 1f stands for 100%, so in this case we will stretch the UIPanel as big as we make this element.
Create a new UIText that we align centered in our UIPanel.
Register the click action to just the UIPanel. Remember that click actions on children propagate through the parent-child chain, so the click event will end up on our UIPanel OnClick handler.
By updating the UIText's text during Update, we can make our text changes thread-safe and avoid errors if we edit the text while it is being drawn.
Recalculate forces this element and its children to recalculate sizes, padding etc. You should do this if the contents change, such as the text in this case. We can copy the min width and min height as they are calculated from the UIText class in this case during Recalculate().
