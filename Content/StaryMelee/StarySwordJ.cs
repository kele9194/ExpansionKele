using Terraria;
using Terraria.ID;
using Terraria.ModLoader;
using Microsoft.Xna.Framework;
using ExpansionKele.Content.Projectiles;
using ExpansionKele.Content.Buff;
using Terraria.DataStructures;
using System;
using System.Collections.Generic;


namespace ExpansionKele.Content.StaryMelee
{
    public class StarySwordJ : StarySwordAbs
    {
        public override string LocalizationCategory => "StaryMelee";
        public override string setNameOverride =>"星元剑J";
        private const string introduction ="星元剑I的升级版,是原版月球领主后的最终升级版";

        // 重写基础属性
        public override int BaseDamage => 111;
        public override int UseTime => 21;
        public override int Rarity => ItemRarityID.Red;
        public override int Crit => 15;
        public override bool UseTurn => false;
        public override int ShootSpeed => 10;
        public override float KnockBack => 8f;

        // 重写增益属性
        protected override int BoostTime => 30;
        protected override int DefenseBoost => 25;
        protected override int LifeRegenBoost => 7 * 20;
        protected override float SpeedBoost => 0.22f;
        protected override float EnduranceBoost => 0.13f;
        public override int WingTimeBoost => 4;
        
        // 重写冲刺属性
        protected override float DashSpeed => 36f;
        protected override int DashDuration => 10;
        protected override int DashIFrames => 40;
        protected override float ManaCostFactor => 0.33f;
        protected override float DefaultGravity => 0.4f;

        public override void SetStaticDefaults()
        {
            //ItemID.Sets.ItemsThatAllowRepeatedRightClick[base.Item.type] = true;
        }

        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            // 添加自定义的 tooltip  
            // TooltipLine line = new TooltipLine(Mod, setNameOverride, introduction);
            // tooltips.Add(line);
        }

        public override void AddRecipes()
        {
            // 创建 GaSniperA 武器的合成配方  
            Recipe recipe = Recipe.Create(ModContent.ItemType<StarySwordJ>()); // 替换为 GaSniperD 的类型  
            recipe.AddIngredient(ItemID.LunarBar, 8);//神圣锭*7
            recipe.AddIngredient(ModContent.ItemType<StarySwordI>(), 1);
            recipe.AddTile(TileID.LunarCraftingStation);
            recipe.Register();
        }
    }
}