...
namespace ExpansionKele.Content.Armor.StarArmorA
{
    ...
    Recipe recipe = Recipe.Create(ModContent.ItemType<StarLeggingsE>()); // 替换为 GaSniperA 的类型  
    recipe.AddRecipeGroup("ExpansionKele:SecondaryBars", 9); // 添加任意金锭组，要求7个 
	//recipe.AddRecipeGroup("ExpansionKele:AnySoul", 1); 
    recipe.AddIngredient(ModContent.ItemType<StarLeggingsD>(), 1);
