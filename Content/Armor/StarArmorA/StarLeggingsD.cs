...
namespace ExpansionKele.Content.Armor.StarArmorA
{
	...
	public class StarLeggingsD : ModItem
	{
		...
		public static string setNameOverride="星元护胫D";
    ...
    Recipe recipe = Recipe.Create(ModContent.ItemType<StarLeggingsD>()); // 替换为 GaSniperA 的类型  
    recipe.AddRecipeGroup("ExpansionKele:PrimaryBars", 9); // 添加任意金锭组，要求7个  
    recipe.AddIngredient(ItemID.SoulofNight, 3); // 暗影之魂*7
    recipe.AddIngredient(ItemID.SoulofLight, 3); // 光明之魂*7
	recipe.AddIngredient(ModContent.ItemType<StarLeggingsC>(), 1); 
